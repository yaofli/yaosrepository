...
YStore.CrossSellRecs = (function() {
	...
	function jsonScriptRequest(fullUrl) {
		...
		createEl('script', {
			...
			src : fullUrl,
			id : ids.script,
			parent : document.getElementsByTagName("head").item(0)
		});
	}
	...
	function createEl(elementType, attr) {
		...
		if (attr.id)
			newEl.id = attr.id;
		if (attr.className)
			newEl.className = attr.className;
		if (attr.title)
			newEl.title = attr.title;

		if (attr.colSpan)
			newEl.colSpan = attr.colSpan;
		if (attr.href)
			newEl.setAttribute('href', attr.href);
		if (attr.type)
			newEl.setAttribute('type', attr.type);
		if (attr.value)
			newEl.setAttribute('value', attr.value);
		if (attr.src)
			newEl.setAttribute('src', attr.src);
		if (attr.alt)
			newEl.setAttribute('alt', attr.alt);
		if (attr.border)
			newEl.setAttribute('border', attr.border);
		if (attr.text)
			newEl.innerHTML = attr.text;
		if (attr.onclick)
			newEl.onclick = attr.onclick;

		if (attr.parent && attr.insertBefore)
			attr.parent.insertBefore(newEl, attr.insertBefore);
		else if (attr.parent)
			attr.parent.appendChild(newEl);
	...
	function requiredCapabilitiesPresent() {
		return (document && document.getElementById
				...
				&& document.getElementsByTagName("head").item(0).appendChild && document
				.getElementsByTagName("head").item(0).setAttribute);
	}
	...
	function assembleUrl() {
		return YStore.crossSellUrl
				+ ((YStore.crossSellUrl.match(/\?/)) ? '&' : '?')
				+ 'callback=YStore.CrossSellRecs.asyncCallback&noCacheIE='
				+ (new Date()).getTime();
	}

	function assembleRecommendations(productArray, headerText, orientation,
			priceLabels) {
		...
		if (orientation !== 'h' && orientation !== 'v') {
			orientation = (typeof YStore.page !== 'undefined' && YStore.page === 'p') ? 'h'
					: 'v';
		}
		var cellsPerRow = orientationSettings[orientation].cellsPerRow;

		if (priceLabels.price === '')
			priceLabels.price = defaultText.price;
		if (priceLabels.salePrice === '')
			priceLabels.salePrice = defaultText.salePrice;
		if (priceLabels.csDiscPrice === '')
			priceLabels.csDiscPrice = (typeof YStore.page !== 'undefined' && YStore.page === 'p') ? defaultText.discountPriceProd
					: defaultText.discountPriceCart;

		var table = createEl('table', {
			className : classNames.table + ' '
					+ classNames.tableOrientation[orientation],
			parent : container
		});
		var tableBody = createEl('tbody', {
			parent : table
		});
		var titleRow = createEl('tr', {
			className : classNames.titleRow,
			parent : tableBody
		});
		var titleCell = createEl('td', {
			colSpan : cellsPerRow,
			className : classNames.titleCell,
			parent : titleRow
		});
		...
		var funcsAvailable = (typeof YStore.CrossSellBeacon != 'undefined'
				&& typeof YStore.CrossSellBeacon.setClickTagUrl === 'function' && typeof YStore.CrossSellBeacon.addRecData === 'function');
		...
		for (var i = 0, len = productArray.length; i < len; i++) {
			...
			if (i % cellsPerRow === 0) {
				...
				if (i === 0)
					classNameArray.push(classNames.firstRow);
				if (len - i <= cellsPerRow)
					classNameArray.push(classNames.lastRow);
				var row = createEl('tr', {
			...
			if (i % cellsPerRow === 0)
				classNameArray.push(classNames.firstCell);
			if ((i + 1) % cellsPerRow === 0)
				classNameArray.push(classNames.lastCell);
			var cell = createEl('td', {
				className : classNameArray.join(' '),
				id : ids.itemPrefix + p.id,
				parent : row
			});
			...
			var caption = createEl('div', {
				className : classNames.caption,
				parent : cell
			});
			var prodName = createEl('h4', {
				className : classNames.name,
				parent : caption
			});
			var prodLink = createEl('a', {
				id : prodLinkId,
				href : p.u,
				text : p.n,
				parent : prodName
			...
			if (showOrderButton)
				(function() {
					var orderUrl = (typeof p.op !== 'undefined' && p.op === 0) ? // Set
																					// the
																					// Order
																					// button
																					// URL
					...
					YStore.addItemUrl
							...
							+ YStore.sessionInfo
							: p.u;
					var orderButtonContainer = createEl('div', {
						className : classNames.orderButtonContainer,
						parent : cell
					});
					if (typeof YStore.orderButtonFormatStr != 'undefined') {
						orderButtonContainer.innerHTML = YStore.orderButtonFormatStr
								.replace(/%s/, orderUrl).replace(/%s/,
										orderButtonId);
					} else {
						var orderButton = createEl('input', {
							...
							parent : orderButtonContainer
						});
					}
				})();

			if (typeof p.pt !== 'undefined' && p.pt !== 'r'
					&& typeof p.rp !== 'undefined' && p.rp !== ''
					&& p.p != p.rp) {
				var prodRegPrice = createEl('p', {
					className : classNames.regularPrice,
					text : priceLabels.price + ' ' + YStore.currencySymbol
							+ p.rp,
					parent : caption
				});
				var prodPrice = createEl('p', {
					className : classNames.price
							+ ' '
							+ ((p.pt === 'c') ? classNames.csDiscPrice
									: classNames.salePrice) + ' '
							+ classNames.promo,
					text : ((p.pt === 'c') ? priceLabels.csDiscPrice
							: priceLabels.salePrice)
							+ ' ' + YStore.currencySymbol + p.p,
					parent : caption
				});
			} else { // This item has only a regular price.
				var prodPrice = createEl('p', {
					className : classNames.price + ' '
							+ classNames.regularPrice,
					text : priceLabels.price + ' ' + YStore.currencySymbol
							+ p.p,
					parent : caption
				});
			...
			if (funcsAvailable) {
				if (imageExists) {
					YStore.CrossSellBeacon.addRecData(ids.imgLinkPrefix + p.id,
							p.u, p.id, p.m, p.s, p.v);
					YStore.CrossSellBeacon.setClickTagUrl(imgLinkId);
				}

				YStore.CrossSellBeacon.addRecData(ids.nameLinkPrefix + p.id,
						p.u, p.id, p.m, p.s, p.v);
				YStore.CrossSellBeacon.setClickTagUrl(prodLinkId);

				// order button
				YStore.CrossSellBeacon.addRecData(ids.orderButtonPrefix + p.id,
						p.u, p.id, p.m, p.s, p.v);
	...
	function callRecViewTag() {
		if (recViewTagCalled || typeof YStore.CrossSellBeacon === 'undefined')
			return;
		if (typeof YStore.CrossSellBeacon.rid === 'undefined')
			YStore.CrossSellBeacon.rid = '';
		if (typeof YStore.CrossSellBeacon.sn === 'undefined')
			YStore.CrossSellBeacon.sn = '';
	...
	return {
		retryAssembleRecs : function() { // Called after a delay if the
											// container element isn't found
											// yet.
			retryAttempts++;
			log('Retry attempt ' + retryAttempts + ' out of ' + RETRY_LIMIT
					+ ', after a delay of ' + RETRY_WAIT + ' ms');
			assembleRecommendations(cached.recsArray, cached.header,
					cached.orientation, cached.priceLabels);
		},
		...
		asyncCallback : function(obj) {
			...
			// r: recommendations, an array of the products that have been
			// recommended for this product page.
			// id: product id.
			// n: product name.
			// p: product price.
			// u: product URL.
			...
			// m: model name.
			// s: sequence.
			// rid: Used in beaconing, must be assigned to
			// YStore.CrossSellBeacon.rid.
			// sn: Used in beaconing, must be assigned to
			// YStore.CrossSellBeacon.sn.
			if (typeof obj !== 'undefined' && typeof obj.r !== 'undefined'
					&& typeof obj.h !== 'undefined'
					&& typeof obj.o !== 'undefined') {
				cached.recsArray = obj.r;
				cached.header = obj.h;
				cached.orientation = obj.o;
				cached.priceLabels = { // We don't check for these because they
										// might not be set.
					price : obj.rpl,
					salePrice : obj.spl,
					csDiscPrice : obj.cpl
		...
		init : function() {
			if (requiredVarsSet() && requiredCapabilitiesPresent()
					&& requiredElementPresent()) {
				if (typeof YStore.csellData !== 'undefined'
						&& typeof YStore.csellData.r !== 'undefined'
						&& typeof YStore.csellData.h !== 'undefined'
						&& typeof YStore.csellData.o !== 'undefined') { // The
																		// data
																		// is
																		// already
																		// present.
					...
					cached.priceLabels = { // We don't check for these because
